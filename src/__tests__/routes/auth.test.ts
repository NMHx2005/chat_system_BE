...
describe('Auth Routes', () => {
    ...
    describe('POST /auth/refresh', () => {
        it('should refresh token successfully', async () => {
            const newToken = generateTestToken(testUser);

            MockedAuthController.refreshToken.mockImplementation(async (req, res) => {
                res.status(200).json({
                    success: true,
                    data: {
                        token: newToken,
            ...
            expect(MockedAuthController.refreshToken).toHaveBeenCalled();
        });
